...
class SimpleCalendar:
    updater = locale.setlocale(locale.LC_ALL, 'uk_UA.utf8')
    async def start_calendar(
        ...
        inline_kb.row()
        inline_kb.insert(InlineKeyboardButton(
            "< місяць", callback_data=calendar_callback.new("PREV-MONTH", year, month, day)
        ))
        inline_kb.insert(InlineKeyboardButton("сьогодні", callback_data='today'))
        inline_kb.insert(InlineKeyboardButton(
            "місяць >", callback_data=calendar_callback.new("NEXT-MONTH", year, month, day)
        ))
